                ...
                <div class="row">
                    <div class="col-lg-12 ">
                        <ul class="nav nav-tabs justify-content-center mt-5">
                            ...
                            <li><a class="nav-link" data-toggle="tab" id= 'user_transaction' href="#transaction">Transactions</a></li>
                        </ul>
                    </div>
                </div>
            </div>
                                            ...
                                            <form id="profile-details-address">
                                                ...
                                                <div class="row">
                                                    <div class="form-group col-md-6">
                                                        ...
                                                        <select class="form-control" id="stu_bil_state_code" required>
                                                            <!-- <option value="India">India</option> -->
                                                            <?php  
                                                            foreach ($state_code as $row) {?>
                                                                <option value="<?php echo $row['state_code_id']; ?>" ><?php echo $row['state_code']; ?></option>
                                                            <?php }?>
                                                        </select>
                                                    </div>
                                                    <div class="form-group col-md-6">
...
<script type="text/javascript">
   function show_key(key_id) {
        ...
        $.ajax({
            ...
            success:function(data)
            {
                if(data != 0){
                    ...
                    $('#stu_bil_state_code').val(data.state_code_value.state_code_id);
                }
    ...
    $('#user_transaction').click(function(e){
         e.preventDefault();
         $.ajax({
            url:'<?php echo base_url(); ?>student/user_transaction',
            dataType:'JSON',
            success:function(data)
            { 
    ...
    $('#user_feedback').click(function(e){
         e.preventDefault();
         $.ajax({
            url:'<?php echo base_url(); ?>student/fetch_orders/feedback',
            dataType:'JSON',
            success:function(data)
            { 
                if(data.msg=='success')
                {
                    $('#feedback').html(data.order_data);
                }
                else
                {
                    $('#feedback').html('<label class="text-center">No Orders Placed Yet.</label>');
                }
