	 	...
	 	if(isset($_FILES['file']['name'])){
			...
			while(($filesop = fgetcsv($handle, 1000, ",")) !== false)
			{
				if($c<>0){	/* SKIP THE FIRST ROW */
					...
					$course_duration = empty($course_duration_db) ? "invalid" : $filesop[1];
					if($course_duration == 'invalid')
					{
					  $error_log_row['coloumn_name'][]="course_duration";
					  $error_log_row['error_type'][]="duration_error";
					...
					if($course_view == 'unknown')
					{
						...
						$error_log_row['error_type'][] =	"mode_error";
						$error_log_row['message'][] =	$filesop[2]." Invalid Course View ID";
					}
 
					$course_validity = $course_duration != "invalid" ? $this->basic_mol->get_single_data(array('course_type_value_name'=>(int)$filesop[1]),'course_type_value_master') : "";
					...
					if(!empty($key_exist_or_not_result))
					{
						...
						$error_log_row['error_type'][]="course_key_exist";
