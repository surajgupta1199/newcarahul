...
class Student_management_model extends CI_Model{ 
	...
	public function _get_datatables_query($data,$check)
	{
		if($data == 'student_detail'){
		    if($check['where2'] != "" && $check['where2'] != null){
				...
				$this->db->select('student_master.*,student_billing_address.*,date_format(student_master.student_date_time, "%d %b %Y %r") as created_at')
						 ->from('student_master')
						 ->where($check['where'])
	...
	public function get_all_ordered_key($where,$table){
		return $this->db->select(''.$table.'.*,atr1.value as mode,atr2.course_view_value_name as view,atr3.course_type_value_name as month')->from($table)
									 ->where($where)
									 ->join('attribute_value_master as atr1',''.$table.'.course_mode = atr1.value_id')
									 ->join('course_view_value_master as atr2',''.$table.'.course_view = atr2.course_view_value_id')
