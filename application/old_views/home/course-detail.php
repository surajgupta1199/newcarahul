								...
								<div class="row mt-3">
									<div class="col-md-6 col-sm-12 mt-3">
										...
										<div class="custom_select">
											<select id="course_price_mode" class="form-control change_config">
												
												<?php 
													foreach($mode_drp_data as $modes){
												?>
													<option value="<?php echo $modes['value_id']; ?>"><?php echo $modes['value'] ?></option>
									...
									<div class="col-md-6 col-sm-12 mt-3">
										...
										<div class="custom_select">
											<select id="course_price_view" class="form-control change_config">
												...
												?>
													<option value="<?php echo $views['value_id']; ?>"><?php echo $views['value'] ?></option>
...
<script type="text/javascript">
    ...
    function onload_set_prices()
	{
		var course_id=$('#course_id').attr('course_id');
		var course_type=$('#course_id').attr('course_type');
		$.ajax({
		      url:"<?php echo base_url(); ?>Home/set_prices_onload", 
		      method:"POST",
		      data:{'course_id': course_id,'course_type':course_type},     
		      dataType: 'json',     
		      success:function(result)
		      { 
		      	$('#course_plan_drp').val(result.course_type_value_id).change();
				$('#course_price_mode').val(result.course_mode).change();
				$('#course_price_view').val(result.course_view).change();
				setTimeout(function(){ 
			      	$('#main_price').text(result.course_price);
			      	$('#discounted_price').text(result.course_dicounted_price);
			      	$('#save_price').text(result.course_save_price);
		      ...
		      {  
		      	if(result.msg == 'success')
		      	{
		      		...
		      		$("#btn_addCart").addClass('btn-primary');
    		       	swal('Info!!',product_quantity+' Product added to cart','success');
