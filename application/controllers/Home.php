...
class Home extends CI_Controller{
	...
	function product_detail(){
		...
		$product_view = $this->basic_mol->get_all_data(array('course_view_id' =>  $product_result[0]['course_type_id'],),"course_view_value_master");
		...
		$where_prod_type = array(
								'course_id'=>$product_id,
								'course_type'=>$prod_type);
		//min product price
		$price_res = $this->student_modl->get_min_prod_price($where_prod_type,'course_price_master');
		$data['min_price_prod'] = $price_res;

		//product Mode
		$prod_mode = $this->student_modl->distinct_type('distinct(course_mode)',$where_prod_type,'course_price_master');	//distinct attr,where,table
		$data['mode_drp_data'] = $this->fetch_array_prod_type($prod_mode,'attribute_value_master','course_mode','value_id');

		//Product Plan
		$where_prod_type['course_mode'] = $price_res['course_mode'];
		$prod_plan = $this->student_modl->distinct_type('distinct(course_type_value_id)',$where_prod_type,'course_price_master');	//distinct attr,where,table
		$data['plan_drp_data'] = $this->fetch_array_prod_type($prod_plan,'course_type_value_master','course_type_value_id','course_type_value_id');

		//Product View
		$prod_view = $this->student_modl->distinct_type('distinct(course_view)',$where_prod_type,'course_price_master');	//distinct attr,where,table
		$data['views_drp_data'] = $this->fetch_array_prod_type($prod_view,'course_view_value_master','course_view','course_view_value_id');
	...
	function set_product_validity_with_mode(){
		$where = $this->input->post();
		$data['min_prod_price'] = "";
		$price_res = $this->student_modl->get_min_prod_price(array('course_id'=>$where['course_id'],'course_type'=>$where['course_type']),'course_price_master');
		if($price_res['course_mode'] == $where['course_mode']){
			$data['min_prod_price'] = $price_res;
		}

		//Product Plan
		$prod_plan = $this->student_modl->distinct_type('distinct(course_type_value_id)',$where,'course_price_master');	//distinct attr,where,table
		$data['plan_drp_data'] = $this->fetch_array_prod_type($prod_plan,'course_type_value_master','course_type_value_id','course_type_value_id');

		//Product View
		$prod_view = $this->student_modl->distinct_type('distinct(course_view)',$where,'course_price_master');	//distinct attr,where,table
		$data['views_drp_data'] = $this->fetch_array_prod_type($prod_view,'course_view_value_master','course_view','course_view_value_id');
		echo json_encode($data);
	}

	function fetch_array_prod_type($prod_type,$prod_table,$search,$from_search){
		$result = array();
		foreach ($prod_type as $value) {
			$result[] = $this->basic_mol->get_single_data(array($from_search=>$value[$search]),$prod_table);
		}
		return $result;	
	}
				                ...
				                ' <li class="grid_item">
				                        <div class="content_box radius_all_10 box_shadow1">
				                            ...
				                            <div class="content_desc">
				                               ...
				                               <li><a class="rating" data-id="1" data-toggle="tooltip" data-placement="top"  title="Add Review">Write a review <i class="ti-comment-alt" onclick="writeReview('.$row["".$table."_id"].',1)"></i></a></li>
				                                    </ul>
				                                </div>
				                            </div>
